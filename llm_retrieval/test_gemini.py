...
entries = entries[15:16]  # for testing
...
def build_user_message(query_id, query_text, candidate_docs):
    system_message = (
        ...
        "You are precise, concise, and return only the JSON output following the schema.\n\n"
    )
    user_message = (
    ...
    user_message += (
        f"Only return a JSON object in this format:\n"
        f'{{\n  "query_id": "{query_id}",\n  "relevant_document_ids": ["1234", "5678", ...]\n}}\n\n'
        f"- Use only document IDs from the list above.\n"
        f"- If no document is relevant, return an empty list.\n"
        f"- Do not invent or hallucinate IDs.\n"
        f"- Ensure every ID you return matches one of the actual document IDs above.\n"
        f"- Do not include explanations or extra text — just return the JSON object."
    )
