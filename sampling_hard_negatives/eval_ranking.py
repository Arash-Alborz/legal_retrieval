...
def evaluate_language(lang):
    ranks_file = f"ranks/e5_reranked_{lang}.json"
    gold_file = f"gold_standard_{lang}.json"
    output_file = f"results/e5_rerank_eval_{lang}.txt"
    ...
    with open(output_file, "w", encoding="utf-8") as f_out:
        f_out.write(f"E5 Reranking Evaluation Results for language: {lang.upper()}\n")
        print(f"\nE5 Reranking Evaluation Results for language: {lang.upper()}")
